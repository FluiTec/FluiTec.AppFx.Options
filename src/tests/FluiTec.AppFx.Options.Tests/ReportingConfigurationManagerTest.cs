    ...
    public class ReportingConfigurationManagerTest : ValidatingConfigurationManagerTest
    {
        ...
        public void CanReportSimpleSettings()
        {
            ...
            Assert.IsTrue(_reportEntries.Contains(string.Format(manager.PropertyReport,
                nameof(OptionWithDefaultKey.StringSetting), stringSetting)));
        }
        ...
        public void CanReportInheritedSettings()
        {
            ...
            var builder = new ConfigurationBuilder()
                ...
                {
                    new KeyValuePair<string, string>(
                        $"{nameof(InheritedOption)}:{nameof(InheritedOption.StringSetting)}", stringSetting),
                    new KeyValuePair<string, string>(
                        $"{nameof(InheritedOption)}:{nameof(InheritedOption.StringSetting2)}", stringSetting2)
                });
            var config = builder.Build();
            var manager = GetManager(config) as ReportingConfigurationManager;
            Assert.IsNotNull(manager);
            ...
            Assert.IsTrue(_reportEntries.Contains(string.Format(manager.PropertyReport,
                nameof(InheritedOption.StringSetting), stringSetting)));
            Assert.IsTrue(_reportEntries.Contains(string.Format(manager.PropertyReport,
                nameof(InheritedOption.StringSetting2), stringSetting2)));
        }
        ...
        public void CanReportNestedSettings()
        {
            ...
            var builder = new ConfigurationBuilder()
                ...
                {
                    new KeyValuePair<string, string>(
                        $"{nameof(NestingOption)}:{nameof(NestingOption.Test)}:{nameof(OptionWithDefaultKey.StringSetting)}",
                        stringSetting)
                });
            var config = builder.Build();
            var manager = GetManager(config) as ReportingConfigurationManager;
        ...
        public void RedactsSimpleSecrets()
        {
            ...
            var builder = new ConfigurationBuilder()
                .AddInMemoryCollection(new[]
                {
                    new KeyValuePair<string, string>($"{nameof(SecretOption)}:{nameof(SecretOption.StringSetting)}",
                        stringSetting)
                });
            var config = builder.Build();
            var manager = GetManager(config) as ReportingConfigurationManager;
            ...
            Assert.IsTrue(_reportEntries.Contains(string.Format(manager.PropertyReport,
                nameof(SecretOption.StringSetting), manager.RedactedValueReplacement)));
        }
        ...
        public void RedactsInheritedSecrets()
        {
            ...
            var builder = new ConfigurationBuilder()
                ...
                {
                    new KeyValuePair<string, string>(
                        $"{nameof(InheritedSecretOption)}:{nameof(InheritedSecretOption.StringSetting)}", stringSetting)
                });
            var config = builder.Build();
            var manager = GetManager(config) as ReportingConfigurationManager;
            Assert.IsNotNull(manager);
            ...
            Assert.IsTrue(_reportEntries.Contains(string.Format(manager.PropertyReport,
                nameof(InheritedSecretOption.StringSetting), manager.RedactedValueReplacement)));
        }
        ...
        public void ReportsOnChangedSetting()
        {
            ...
            config.Providers.Single()
                .Set($"{nameof(OptionWithDefaultKey)}:{nameof(OptionWithDefaultKey.StringSetting)}", "test2");
            config.Reload();

            Assert.IsTrue(_reportEntries.Contains(string.Format(manager.SettingsChangedReport,
                nameof(OptionWithDefaultKey))));
        }
    }
}