...
public class OptionsConsoleModule : ModuleConsoleItem
{
    ...
    public override Command ConfigureCommand()
    {
        ...
        editCmd.AddOption(new Option<string>("--value", "(New) Value of the configuration-entry.")
            {IsRequired = false});
        editCmd.Handler = CommandHandler.Create(new Func<string, string, int>(ProcessEdit));
        ...
        addCmd.Handler = CommandHandler.Create(new Func<string, string, int>(ProcessEdit));
        ...
        viewCmd.Handler = CommandHandler.Create(new Func<string, int>(ProcessRead));
    ...
    private int ProcessEdit(string key, string val)
    {
        if (EditSetting(key, val))
        {
            AnsiConsole.MarkupLine(
                $"The value of key '{Presenter.HighlightText(key)}' was set to '{Presenter.HighlightText(val)}'");
